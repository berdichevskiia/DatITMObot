...
bot = Bot(token="6005884016:AAEWegzDFQGdzfWrcn_mGWiOm8cS64nBei0")
...
def show_anketa(name, age, faculty, text):
    return f'{name}\n{age}\n{faculty}\n{text}'
...
async def age(message: types.Message, state: FSMContext):
    ...
    await Wait.faculty.set()


@dp.message_handler(state=Wait.faculty)
async def faculty(message: types.Message, state: FSMContext):
    if len(message.text) > 30:
...
async def download_photo(message: types.Message, state: FSMContext):
    ...
    d = list(data.values())
    print(list(map(lambda x: str(x).encode('utf-8'), d)))
...
async def anketa_reaction(message: types.Message, state: FSMContext):
    if message.text == "Лайк":
        ...
        list_of_anketi = BotDB.find_anketi(message.from_user.id, data["interest"], data["faculty"], data["age"])
        ...
        await bot.send_message(text=f"Начинай общатся, если понравлися(лась) - @{message.from_user.username}",
                               chat_id=liked_id)
...
async def delete_confirm(message: types.Message, state: FSMContext):
    if message.text == "Да":
        ...
        await message.answer("Ваша анкета удалена!\nВы можете вернутся сюда в любое время по команде /start",
